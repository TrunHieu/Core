...
int main(void)
{
  ...
  hx711_coef_set(&loadcell, 475); // read afer calibration
...
//LCD-Keypad 4x4 -----------------------------------------------
	  ...
	  if(buffer[key] != 0 && buffer[key] != 'D' && buffer[key] != 'C'&& buffer[key] != 'A')
	  {
		  lcd_gotoxy(0,2);
	  	  lcd_send_string("Price :");
		  lcd_gotoxy(shift, 2);
	  ...
	  //----------------------------Tính tổng ti�?n khi nhán nút D --------------------------
	  	  if(buffer[key] == 'D')
	  	  {
	  		 Total = (buffer[0] - 48)*10+(buffer[1] - 48)*1; //buffer[0] - 48 là để chuyển giá trị char v�? giá trị int (Tham khảo bảng mã ASCII)
	  		//Total = (buffer[0] - 48)*10000 + (buffer[1] - 48)*1000+ (buffer[2] - 48)*100 + (buffer[3] - 48)*10 + (buffer[4] - 48)*1;	/*  B�? comment dòng này để có thể tính số ti�?n hàng trăm, chục, đơn vị  */
	  		  Total = Total * weight;
...
//------------------------- Xóa giá ti�?n khi nhấn nút C----------------------------
	  if(buffer[key] == 'C')
	  {
		  shift = 8;					// �?ưa con tr�? vị trí giá đầu tiên v�? vị trí 7
	  ...
	  if(buffer[key] == 'A')weight = setweight;
	  {
		  lcd_gotoxy(8,1);
		  sprintf(LCD_send, "%.3f", weight);  //weight
		  lcd_send_string(LCD_send);
